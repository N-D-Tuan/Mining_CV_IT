...
export default function Job_Detail() {
  ...
  async function fetchAppliedStatus(jobId) {
    try {
      const res = await fetch(
        `${API_BASE}/api/v1/users/applied-jobs/${jobId}`,
        {
          credentials: "include",
        },
      );
  ...
  async function fetchRelated(jobData) {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok)
        setRelated((data.data?.items || []).filter((j) => j.id !== jobData.id));
    } catch (e) {
  ...
  if (error) {
    return (
      <main className="max-w-container-max mx-auto px-gutter py-xl">
        <div className="p-lg rounded-xl bg-surface-container-lowest border border-red-200 text-red-700">
          {error}
        </div>
      </main>
    );
        ...
        <div className="w-full lg:w-[70%] space-y-lg">
          <header className="bg-surface-container-lowest p-xl rounded-xl shadow-sm border border-outline-variant">
            <div className="flex flex-col md:flex-row justify-between items-start gap-md">
              <div className="space-y-sm">
                ...
                <div className="flex flex-wrap items-center gap-md text-on-surface-variant">
                  <span className="flex items-center gap-xs">
                    ...
                    {job?.employer_name || job?.source || "Nhà tuyển dụng"}
                  </span>
                  <span className="flex items-center gap-xs">
                    <span className="material-symbols-outlined text-[18px]">
                      schedule
              ...
              <div className="bg-primary-container/10 text-primary-container px-md py-sm rounded-lg font-bold text-headline-sm">
                {formatSalary(job)}
              </div>
            </div>
          </header>
          ...
          <div className="flex border-b border-outline-variant gap-xl">
            <button
              ...
              className={`pb-md font-label-md text-label-md ${tab === "description" ? "border-b-2 border-primary text-primary font-bold" : "text-on-surface-variant hover:text-primary"}`}
            >
              Mô tả công việc
            </button>
            <button
              onClick={() => setTab("requirements")}
              className={`pb-md font-label-md text-label-md ${tab === "requirements" ? "border-b-2 border-primary text-primary font-bold" : "text-on-surface-variant hover:text-primary"}`}
            >
              Yêu cầu
            </button>
            <button
              onClick={() => setTab("benefits")}
              className={`pb-md font-label-md text-label-md ${tab === "benefits" ? "border-b-2 border-primary text-primary font-bold" : "text-on-surface-variant hover:text-primary"}`}
            >
              Quyền lợi
            </button>
          </div>
          ...
          <article className="bg-surface-container-lowest p-xl rounded-xl shadow-sm border border-outline-variant space-y-xl">
            {tab === "description" && (
              <section>
                <h2 className="font-display text-headline-sm font-bold text-on-surface mb-md">
                  Mô tả công việc
                </h2>
                <div
                  className="text-on-surface-variant leading-relaxed font-body-md text-body-md"
                  dangerouslySetInnerHTML={{
                    __html:
                      job?.post_content || job?.raw_salary || "Không có mô tả",
                  }}
                />
              </section>
            )}
                ...
                {Array.isArray(job?.requirements) && job.requirements.length ? (
                  <ul className="space-y-sm">
                    {job.requirements.map((r, i) => (
                      <li
                        key={i}
                        className="flex items-start gap-sm text-on-surface-variant"
                      >
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
                  ...
                  <div className="p-md bg-surface-container-low rounded-lg border border-outline-variant/30 flex items-center gap-md">
                    <span className="material-symbols-outlined text-primary bg-primary/10 p-sm rounded-full">
                      payments
                    </span>
                    <span className="font-label-md text-label-md">
                      {job?.salary_type || "Lương thưởng cạnh tranh"}
                    </span>
                  </div>
                  <div className="p-md bg-surface-container-low rounded-lg border border-outline-variant/30 flex items-center gap-md">
                    <span className="material-symbols-outlined text-primary bg-primary/10 p-sm rounded-full">
              ...
              <div className="flex items-center gap-md">
                <div>
                  ...
                  <p className="text-on-surface-variant font-body-sm text-body-sm">
                    {job?.employer_name ? `${job.employer_name}` : "Recruiter"}
                  </p>
                </div>
              </div>
              <button
          ...
          <div className="sticky-sidebar space-y-lg">
            <div className="bg-surface-container-lowest p-xl rounded-xl shadow-sm border border-outline-variant space-y-md">
              <button
                ...
                onClick={handleApply}
                className="w-full bg-primary-container text-on-primary-container py-md rounded-xl font-bold flex items-center justify-center gap-sm hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
    ...
    "
                  >
                    <img
                      alt="LinkedIn"
                      className="w-6 h-6 object-contain"
                      src="/html_css_js/linkin_icon.png"
                    />
                  </button>
    ...
    "
                  >
                    <img
                      ...
                      src="/html_css_js/instargram_icon.jpg"
                    />
                  </button>
                </div>
              </div>
            ...
            <div className="bg-primary/5 p-lg rounded-xl border border-primary/20">
              <div className="flex items-center gap-sm text-primary mb-sm">
                ...
                <span className="font-bold font-label-md text-label-md">
                  Mẹo an toàn
                </span>
              </div>
              <p className="font-body-sm text-body-sm text-on-surface-variant">
                Không bao giờ trả bất kỳ khoản phí nào để xin việc. Hãy báo cáo
                nếu thấy dấu hiệu nghi ngờ.
              </p>
            </div>
          </div>
        </aside>
      ...
      <section className="mt-3xl">
        <div className="flex justify-between items-center mb-xl">
          ...
          <Link
            to="/jobs"
            className="text-primary font-bold flex items-center gap-xs hover:gap-md transition-all"
          >
            Xem tất cả{" "}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-lg">
          {related.map((r) => (
            <Link
              key={r.id}
              to={`/jobs/${r.id}`}
              className="bg-surface-container-lowest p-lg rounded-xl shadow-sm border border-outline-variant hover:scale-[1.01] transition-transform duration-200 block"
            >
              <div className="flex items-start gap-md mb-md">
                <div>
                  <h3 className="font-headline-sm text-headline-sm font-bold line-clamp-1">
                    {r.title}
                  </h3>
                  <p className="text-on-surface-variant font-body-sm text-body-sm">
                    {r.employer_name || r.source}
                  </p>
                </div>
              </div>
              <div className="space-y-md">
                ...
                <div className="flex items-center justify-between">
                  <span className="bg-secondary-container text-on-secondary-container px-sm py-xs rounded text-label-sm font-label-sm">
                    {r.job_type || "Part-time"}
                  </span>
                  <span className="text-primary font-bold">
                    {r.min_salary || r.raw_salary || "-"}
                    {r.max_salary ? ` - ${r.max_salary}` : ""}
                  </span>
                </div>
              </div>
            </Link>
