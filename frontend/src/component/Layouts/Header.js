                            ...
                            {isDropdownOpen && (
                                <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-surface-dim border border-outline-variant rounded-xl shadow-lg overflow-hidden z-50">
                                    ...
                                    <div className="p-4 border-b border-outline-variant flex justify-between items-center">
                                        ...
                                        {unreadCount > 0 && (
                                            <button 
                                                onClick={() => setNotifications(prev => prev.map(n => ({...n, is_seen: true})))}
                                    ...
                                    <ul className="max-h-[60vh] overflow-y-auto">
                                        {notifications.length === 0 ? (
                                            <li className="p-4 text-center text-on-surface-variant text-sm">Chưa có thông báo nào</li>
                                        ) : (
                                            notifications.map(notif => (
                                                <li key={notif.id} onClick={() => handleNotificationClick(notif.id, notif.job_id)} className={`cursor-pointer p-4 border-b border-outline-variant transition-colors ${!notif.is_seen ? 'bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100' : 'hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                                                    <div className="text-sm text-on-surface">
                                                        {!notif.is_seen && <span className="text-primary font-bold mr-2">● Mới</span>}
